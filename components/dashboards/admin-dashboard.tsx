  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Total Classes", value: String(CLASSES.length), icon: BookOpen, color: "indigo" },
          { label: "Students", value: String(students.length), icon: GraduationCap, color: "emerald" },
          { label: "Instructors", value: String(teachers.length), icon: Users, color: "violet" },
          ...
          return (
            <Card key={s.label}>
              <CardContent className="p-4 flex items-center gap-3">
                <div className={cn("w-9 h-9 rounded-lg flex items-center justify-center shrink-0", colorMap[s.color])}>
          ...
          <CardContent className="space-y-3 pt-0">
            {CLASSES.map((cls) => (
              <div key={cls.id} className="flex items-center gap-3">
                <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-xs shrink-0", CLASS_BG[cls.color] ?? "bg-primary")}>
                  {cls.code.slice(0, 2)}
                </div>
          ...
          <CardContent className="space-y-3 pt-0">
            {USERS.slice(0, 6).map((user) => (
              <div key={user.id} className="flex items-center gap-3">
                ...
                <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full leading-none capitalize", ROLE_BADGE[user.role])}>
                  {user.role}
                </span>
              </div>
            ))}
